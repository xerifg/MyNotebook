...
* [视频解释线性代数的本质](https://www.bilibili.com/video/BV1ys411472E/?spm_id_from=333.999.0.0&vd_source=8c065934da63850a7afd383a2017d263) 
  ...
  核心思想：**矩阵描述的是一场变换 ，变换的本质是基的变换**，因为所有向量都是基的组合体
    ...
    * 矩阵的**行列式**描述了坐标系变换后原单位面积被放大了多少倍，行列式为0代表坐标系空间体积被压缩到了0
    * 矩阵的**秩**描述了变换后坐标系的维度
    ...
    * $ A^{-1}MA $ 其中M表示的是线性转换，A矩阵充当翻译的作用（假设A矩阵表示从0坐标系转换到1坐标系的转换矩阵，M矩阵表示在0坐标系下逆时针旋转90°，则$ A^{-1}MAB $表示B向量在1坐标系下逆时针旋转90°后的坐标）
